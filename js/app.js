$(function () {
  ...
  // index.php上で<a href="#store_section">がクリックされたときのアニメーション
  if (location.pathname == "/") {
    (function () {
      $(".link-access").click(function (event) {
        event.preventDefault();
        // 開いたヘッダーを閉じる
        $(".bar-upper-line").removeClass("bar-upper-line-open");
        $(".bar-under-line").removeClass("bar-under-line-open");
        $(".header-nav-list").slideUp();
        $("body,html").animate(
          {
            scrollTop: $("#store_section").offset().top - 100
          },
          500
        );
      });
  ...
  $("#top_caption").bgSwitcher({
    images: [
      "https://sample.sntsz20.com/wp-content/themes/izakaya-theme2/img/food.jpg",
      "https://sample.sntsz20.com/wp-content/themes/izakaya-theme2/img/sashimi.jpg",
      "https://sample.sntsz20.com/wp-content/themes/izakaya-theme2/img/sushi.jpg"
    ], // 切替背景画像を指定
  ...
  var target_adjuctment = [["#top_caption", "#top_caption_inner", 180]];
  ...
  $(".large-img img").height($(".large-img img").width() * 0.6);
  $(window).resize(function () {
    $(".large-img img").height($(".large-img img").width() * 0.6);
  });
  ...
  while ($(".post-oder-" + post_num).length) {
    (function (post_num) {
      $(".post-oder-" + post_num + " .news-detail-head").click(function (event) {
        for (var i = 1; i <= 3; i++) {
          $(
            ".post-oder-" + post_num + " .news-detail-arrow-part" + i
          ).toggleClass("news-detail-arrow-part" + i + "-open");
        }
